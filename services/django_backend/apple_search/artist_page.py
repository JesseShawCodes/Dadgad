...
def artist_content(artist_id):
    ...
    if (
        ...
        or not artist_details.get("artist_id")
        or not artist_details.get("image_url")
    ):
...
def get_artist_high_level_details(artist_id):
    ...
    if (
        not artist_data.get("name")
        or not artist_data.get("artwork")
        or not artist_data["artwork"].get("url")
        or not artist["data"][0].get("id")
    ):
        return {}
...
def top_songs_list_builder(artist_id):
    ...
    if playlists_content and playlists_content.get("data"):
        for song_list in playlists_content.get("data", []):
            if (
                song_list.get("relationships")
                and song_list["relationships"].get("tracks")
                and song_list["relationships"]["tracks"].get("data")
            ):
