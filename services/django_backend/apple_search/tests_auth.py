...
# MAKE LINE SHORTER BY IMPORTING THE FUNCTIONS FROM auth.py
from apple_search.auth import (
    get_auth_token,
    get_newest_auth,
    apple_request,
    log_error,
...
class AppleAuthTests(TestCase):
    ...
    @patch("apple_search.auth.pyjwt.encode")
    @patch("apple_search.auth.os.environ.get")
    def test_get_auth_token_encoding_failure(
        self, mock_env_get, mock_jwt_encode
    ):
    ...
    def test_get_newest_auth(self):
        ...
        # Ensure we have different timestamps if needed, though
        # order_by("-created") usually works fine in tests
    ...
    @patch("apple_search.auth.requests.get")
    @patch("apple_search.auth.get_newest_auth")
    def test_apple_request_expired_token(
        self, mock_get_newest, mock_get, mock_get_token
    ):
