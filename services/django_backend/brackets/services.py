    ...
    def create_bracket(artist_id, artist_name, songs):
        ...
        for i, song in enumerate(selected_songs):
            item = BracketItem.objects.create(
                ...
                image_url=song.get('attributes', {}).get('artwork', {}).get('url', '').replace('{w}', '300').replace('{h}', '300'),
                seed=i + 1
            )
        ...
        for r in range(1, num_rounds):
            for m in range(len(matchups_by_round[r])):
                ...
                matchups_by_round[r][m].next_matchup = matchups_by_round[r+1][next_match_index]
